...
\begin{center}
<<>>=
# Density plot showing distribution of observation quantities
data %>% 
...
length(data$Quantity[data$Quantity<=100])/length(data$Quantity)
length(data$Quantity[data$Quantity<=10])/length(data$Quantity)
length(data$Quantity[data$Quantity<=1])/length(data$Quantity)
@
...
data %>% 
  ...
  ggplot(aes(x = months, y = `Number of observations`)) +
    ...
    ggtitle("Categorisation of Rubber Glover over Time") + 
    theme_bw() +
    theme(plot.title = element_text(size=10),
          panel.grid.major = element_blank(), 
          panel.grid.minor = element_blank(),
...
\begin{figure}[H] 
\begin{center}
<<eval=FALSE, echo=FALSE, fig.height=4>>=
data %>% 
  filter(`Material Description` == "PLASTIC") %>% 
...
data %>%
  ...
  ggplot(aes(x = Year, y = Average_Daily_Observations)) +
    geom_col() +
    ggtitle("Average Daily Observations by Year") +
    xlab("Year") +
...
data %>% 
  ...
  #filter(`Material Description`=="PLASTIC") %>%
  ggplot(aes(x = Year, y = prop, color = `Material Description`, label = lab))+
#  geom_text(color = "black") + 
  geom_line(aes(x = Year, y = prop, color = `Material Description`))+
  geom_point(aes(x = Year, y = prop, color = `Material Description`)) +
  ggtitle("Some Title") + 
...
data %>% 
  ...
  ggtitle("Top 15 Debris Items Observed between 2012 and 2019") +
  xlab("Debris") + 
  ylab("") +
  #coord_flip() +
  theme(text = element_text(size=8),
...
data %>% 
  ...
  arrange(desc(Observed)) %>% 
  head(100) %>%
  ggplot(aes(x = Location, y = Observed, fill = `Material Description`)) +
    geom_col(colour = "black", size = 0.2, position = "fill") +
    ggtitle("Some Title") + 
    scale_y_continuous(labels=scales::percent, name="") +
    theme_bw() +

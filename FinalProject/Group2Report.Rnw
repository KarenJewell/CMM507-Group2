...
Marine pollution is a major global issue which impacts on environment, economy and human health. Although marine pollution is caused by many different materials, plastics consist of 60-80\% of the marine litter. \cite{DERRAIK2002} \cite{SMITH2013} \cite{KUO2014} \cite{ZHOU2011} \cite{JANG2014} Plastics are synthetic organic polymers and their lightweight feature and durability make it very suitable to make a range of products we use in our everyday life.\cite{BARNES2009} \cite{SIVAN2011} These same features are what makes plastic a major component of pollution, due to overuse and non-managed waste disposal systems internationally, contributing to 10\% of the waste generated worldwide.\cite{BARNES2009} As the global production of plastic is increasing every year, the problem in the marine environment is also getting bigger. Due to its buoyancy, plastic debris can be dispersed over long distances and accumulate on the shorelines, even polluting the most remote areas (Lavers and Bond, 2017). Jambeck et al.,\cite{JAMBECK2015} reported that in 2010 alone, between 4.8 million to 12.7 million metric tons of plastics entered the ocean. Plastics are everywhere in the marine environment and urgent action is required to mitigate this problem and reduce its harmful impact.\cite{RIOS2007} \cite{ROCHMAN2015}. \\
...
data %>% 
  ...
  ggplot(aes(y=Quantity)) + 
    geom_boxplot() +
    ggtitle("Quantities Distribution") + 
...
data %>% 
  ...
  ggplot(aes(y=Quantity)) + 
    geom_boxplot() +
    ggtitle("Quantities Distribution excl QTY>10,000") + 
    theme(plot.title = element_text(size=10))

format(summary(data$Quantity), scientific=FALSE) %>%
  xtable()

# Distribution of unique quantities
data %>%
  select(Quantity) %>%
  unique() %>%
...
@
There are 5 observations with quantities of >10,000, 3 of them are cigarettes in 2012, 2013 and 2015. We can't use statistical outliers to remove all outliers as with so many individual entries the IQR is 4.5 and excluding everything above that would exclude all the entries made by organised groups. 
...
# Checking the time of Rubber Glove items
data %>% 
  mutate(year = as.integer(year(Time))) %>% 
  select(`Material Description`, ItemName, Time) %>%
  filter(ItemName == "Rubber Gloves") %>%
  distinct() %>%
  xtable()
@
...
\begin{center}
<<>>=
# Line chart showing categorisation of Rubber Glover over time  
data %>% 
  filter(ItemName == "Rubber Gloves") %>%
...
Further analysis finds that the use of "Rubber" to classify Rubber Gloves only appears in September 2019 and the last categorisation as "Plastic" also at the same time, this suggests that the classification was actually changed around this time, and perhaps suggests to us that we should reclassify all historic Rubber Glove items as Rubber. \hl{we should probably change it to make them consistent or explain why we didn't} \\
...
\hl{Treemap maybe move back into pre-processing section.}
...
\hl{remove scientific scale}

Cigarettes are the most common item recorded as seen in. %Figure~\ref{figI}
This observation does not agree with the India study which found cigarettes was actually their smallest pollutant at 5.5\% \cite{KUMAR2016}. Perhaps some of the debris is not actually from the sea, but rather from people littering by the coastline? Does debris littered on the coastline end up in the oceans? \hl{can we find out more here? smoking rates?}
...
data %>% 
  ...
  select(Latitude, Longitude, Quantity, Location) %>% 
  ggplot() +
...
xtable(correlation_accuracy)

xtable(min_max_accuracy)

...
\hlstd{data} \hlkwb{<-} \hlstd{data} \hlopt{%>%} \hlkwd{filter}\hlstd{(}\hlkwd{as.integer}\hlstd{(}\hlkwd{year}\hlstd{(Time))} \hlopt{%in%} \hlnum{2012}\hlopt{:}\hlnum{2019}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlstd{data} \hlopt{%>%}
  ...
  \hlkwd{pivot_longer}\hlstd{(}\hlkwc{cols} \hlstd{=} \hlkwd{everything}\hlstd{(),} \hlkwc{names_to} \hlstd{=} \hlstr{"Column Name"}\hlstd{,} \hlkwc{values_to} \hlstd{=} \hlstr{"Unique Values"}\hlstd{)} \hlopt{%>%}
  \hlkwd{arrange}\hlstd{(}\hlkwd{desc}\hlstd{(`Unique Values`))} \hlopt{%>%}
  \hlkwd{xtable}\hlstd{()}
\end{alltt}
\end{kframe}% latex table generated in R 3.6.3 by xtable 1.8-4 package
% Sat May 09 16:59:24 2020
\begin{table}[ht]
\centering
\begin{tabular}{rlr}
  \hline
...
##  4 PLASTIC                Rubber Gloves Taste of Omaha Cleanup
##  5 PLASTIC                Rubber Gloves 2 diff kinds          
##  6 PLASTIC                Rubber Gloves undefined             
##  7 PLASTIC                Rubber Gloves Latex                 
##  8 PLASTIC                Rubber Gloves Hose                  
##  9 PLASTIC                Rubber Gloves Cap                   
## 10 PLASTIC                Rubber Gloves Vial                  
## # ... with 20 more rows
\end{verbatim}
\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in readWorksheet(loadWorkbook("{}data/meetings.xlsx"{}), sheet = 1): could not find function "{}readWorksheet"{}}}\begin{alltt}
\hlstd{dfs}\hlopt{$}\hlstd{Date} \hlkwb{<-}\hlkwd{as.character}\hlstd{(dfs}\hlopt{$}\hlstd{Date)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'dfs' not found}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in xtable(dfs, caption = "{}Record of Team Meetings"{}, label = "{}tab:one"{}, : object 'dfs' not found}}\end{kframe}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in readWorksheet(loadWorkbook("{}data/peers.xlsx"{}), sheet = 1): could not find function "{}readWorksheet"{}}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in xtable(dfs, caption = "{}Peer Assessment out of 100"{}, label = "{}tab:two"{}, : object 'dfs' not found}}\end{kframe}

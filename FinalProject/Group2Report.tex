...
\hlstd{data} \hlopt{%>%}
  ...
  \hlkwd{ggplot}\hlstd{()} \hlopt{+}
    \hlkwd{geom_polygon}\hlstd{(}\hlkwc{data} \hlstd{=} \hlkwd{map_data}\hlstd{(}\hlstr{"world"}\hlstd{),}
                 \hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= long,} \hlkwc{y} \hlstd{= lat,} \hlkwc{group} \hlstd{= group),}
                 \hlkwc{fill} \hlstd{=} \hlstr{"grey"}\hlstd{,} \hlkwc{alpha} \hlstd{=} \hlnum{0.5}\hlstd{)} \hlopt{+}
...
\hlkwd{tibble}\hlstd{(}\hlkwc{Quantity} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"<=1"}\hlstd{,}\hlstr{"<=10"}\hlstd{,}\hlstr{"<=100"}\hlstd{),}
       \hlkwc{ObservationProportion} \hlstd{=} \hlkwd{c}\hlstd{(}\hlkwd{percent_quantity}\hlstd{(}\hlnum{1}\hlstd{),}
                                 \hlkwd{percent_quantity}\hlstd{(}\hlnum{10}\hlstd{),}
                                 \hlkwd{percent_quantity}\hlstd{(}\hlnum{100}\hlstd{)))} \hlopt{%>%}
  \hlkwd{xtable}\hlstd{(}\hlkwc{caption} \hlstd{=}
           \hlstr{"Proportion of observation having equal or less than the stated Quantity"}\hlstd{,}
...
\hlstd{recategorise} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{)\{}
  ...
  \hlkwa{if}\hlstd{(x} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlnum{3}\hlstd{,}\hlnum{15}\hlstd{,}\hlnum{9}\hlstd{,}\hlnum{22}\hlstd{))\{out} \hlkwb{=} \hlstr{"Cigarette related waste"}\hlstd{\}}
  \hlkwa{if}\hlstd{(x} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlnum{6}\hlstd{,}\hlnum{11}\hlstd{,}\hlnum{8}\hlstd{,}\hlnum{5}\hlstd{,}\hlnum{10}\hlstd{,}\hlnum{4}\hlstd{,}\hlnum{23}\hlstd{,}\hlnum{14}\hlstd{)) out} \hlkwb{=} \hlstr{"Food related waste"}
  \hlkwa{if}\hlstd{(x} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlnum{17}\hlstd{,}\hlnum{7}\hlstd{,}\hlnum{13}\hlstd{,}\hlnum{21}\hlstd{,}\hlnum{1}\hlstd{,}\hlnum{16}\hlstd{,}\hlnum{20}\hlstd{,}\hlnum{19}\hlstd{)) out} \hlkwb{=} \hlstr{"Other"}
  \hlkwa{if}\hlstd{(x} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlnum{2}\hlstd{,}\hlnum{18}\hlstd{)) out} \hlkwb{=} \hlstr{"Plastic bags and Styrofoam packaging"}
  \hlkwa{if}\hlstd{(x} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlnum{12}\hlstd{,}\hlnum{23}\hlstd{,}\hlnum{24}\hlstd{,}\hlnum{25}\hlstd{)) out} \hlkwb{=} \hlstr{"Fragments"}
...
\hlstd{data} \hlopt{%>%}
  ...
  \hlkwd{full_join}\hlstd{(}\hlkwd{tibble}\hlstd{(}\hlkwc{Date} \hlstd{=} \hlkwd{full_seq}\hlstd{(}\hlkwd{c}\hlstd{(}\hlkwd{min}\hlstd{(}\hlkwd{date}\hlstd{(data}\hlopt{$}\hlstd{Time)),}
                                     \hlkwd{max}\hlstd{(}\hlkwd{date}\hlstd{(data}\hlopt{$}\hlstd{Time))),}
                                   \hlkwc{period} \hlstd{=} \hlnum{1}\hlstd{)),} \hlkwc{by} \hlstd{=} \hlstr{"Date"}\hlstd{)} \hlopt{%>%}
...
\hlstd{data} \hlopt{%>%}
  \hlkwd{filter}\hlstd{(`Material Description`} \hlopt{==} \hlstr{"PLASTIC"}\hlstd{,}
         \hlstd{ItemName} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlstr{"Fireworks"}\hlstd{))} \hlopt{%>%}
